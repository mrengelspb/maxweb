const totalConImpuestos = (productos, iva, total) => {
    ...
    impuestos += `<totalImpuesto>
                ...
                <baseImponible>${parseFloat(total).toFixed(2)}</baseImponible>
                <tarifa>12</tarifa>
                <valor>${parseFloat(iva).toFixed(2)}</valor>
            </totalImpuesto>`;
    return impuestos;
}
           ...
           <precioTotalSinImpuesto>${item.total}</precioTotalSinImpuesto>
            <impuestos>
                <impuesto>
                    ...
                    <baseImponible>${parseFloat(item.total).toFixed(2)}</baseImponible>
                    <valor>${parseFloat((item.total * 12) / 100).toFixed(2)}</valor>
                </impuesto>
            </impuestos>
        </detalle>`;
    }    
